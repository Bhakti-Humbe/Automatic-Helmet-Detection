...
def insertBLOB( biodataFile):
    ...
    try:
        ...
        sql_insert_blob_query = """ INSERT INTO imagevalue
                          (image,date,time) VALUES (%s,%s,%s)"""
        ...
        result = cursor.execute(sql_insert_blob_query, (insert_blob_tuple,today,now))
        connection.commit()
...
def detect(opt):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        pred = non_max_suppression(pred, 0.8, opt.iou_thres, opt.classes, opt.agnostic_nms,
                                   max_det=opt.max_det)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    print("=============",int(conf))
                    if save_img or opt.save_crop or view_img or webcam or conf>90:  # Add bbox to image
                ...
                text_for_warning = "test" #####################################################################
                ## 1. helmet_head_person.pt
                ## 씽씽이 경우 
                iii=0;
                ii=ii+1;
                if(num_person == num_head):
                   text_for_warning = " No Helmet Detected, Please wear th Helmet"
                   iii=1;
